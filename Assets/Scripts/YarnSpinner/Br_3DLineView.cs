...
namespace Yarn.Unity
{
    ...
    /// elements.
    /// </summary>
    public class Br_3DLineView : DialogueViewBase
    {
        // Custom shit
        [System.Serializable]
        public struct NpcInfo
        {
            ...
            public GameObject lineTextContent;
            //public TextMeshProUGUI lineText;
        }
        public List<NpcInfo> npcInfo = new List<NpcInfo>();
        private string characterNameStored;
        public Br_CharacterStyleView characterStyleView;

        public delegate void OnRunLine();
        public static event OnRunLine onRunLine;
        public GameObject newLineObj;
        private string lineTextPrep;
        // end of custom shit
        ...
        /// <summary>
        /// Chuke tests scroll view content
        /// </summary>
        internal GameObject lineTextContent = null;


        private void Awake()
        {
            //canvasGroup.blocksRaycasts = false;
            characterStyleView = FindObjectOfType<Br_CharacterStyleView>();
        }
        ...
        public void UpdateDialogueTargets(string characterName)
        {
            foreach (NpcInfo character in npcInfo)
            {
                if (character.name == characterName)
                {
                    canvasGroup = character.canvasGroup;
                    //lineText = character.lineText;
                    lineTextContent = character.lineTextContent;
                }
            }
        }
        ...
        private IEnumerator DismissLineInternal(Action onDismissalComplete)
        {
            ...
            if (useFadeEffect)
            {
                //yield return StartCoroutine(Effects.FadeAlpha(canvasGroup, 1.0f, 0, fadeOutTime, currentStopToken));
            ...
            // clear any interrupted nodes
            //DialogueManager.Instance.stoppedNode = "";
        ...
        public override void InterruptLine(LocalizedLine dialogueLine, Action onInterruptLineFinished)
        {
            ...
            if (characterNameText == null)
            {
                ...
                {
                    // adds the last line + the next one
            ...
            IEnumerator PresentLine()
            {
                ...
                else
                {
                    ...
                    if (showCharacterNameInLineView){
                        ...
                        // If we have a name stored, and it's the same as the one we're looking at now, run the line without the name
                        if (characterNameStored != null && dialogueLine.CharacterName == characterNameStored){
                            lineTextPrep = "<uppercase><style=" + dialogueLine.CharacterName + ">" + "</uppercase><indent=4.25em><color=#969696>></color></indent><indent=5.75em>" + dialogueLine.TextWithoutCharacterName.Text + "</indent></style>";
                        }
                        else{
                            // otherwise if we don't have a character name, or the name we do have doesn't match the stored name,
                            // run the line with the name and update the stored name
                            lineTextPrep = "\n" + "<uppercase><style=" + characterName + ">" + characterName + "</uppercase><indent=5.75em>" + dialogueLine.TextWithoutCharacterName.Text + "</indent></style>";
                            characterNameStored = dialogueLine.CharacterName;
                        }

                        //lineText.maxVisibleCharacters = int.MaxValue;
                        //Debug.Log("lineText: " + lineTextPrep);
                        GameObject newTextObj = Instantiate(newLineObj, transform.position, Quaternion.identity);
                        newTextObj.transform.SetParent(lineTextContent.transform, false);
                        newTextObj.GetComponent<TextMeshProUGUI>().text = lineTextPrep;

                        // lineTextContent.GetComponentInParent<ScrollViewController>().ScrollToNewLine();

                    // manual style
                        // GameObject newText = new GameObject("newText");
                        // newText.transform.parent = lineTextContent.transform;

                        // TextMeshProUGUI newTextComponent = newText.AddComponent<TextMeshProUGUI>();
                        // newTextComponent.fontSize = 10;
                        // newTextComponent.margin = new Vector4(10, 0, 0, 10);
                        // lineText = newTextComponent;

                        // RectTransform newTextRect = newText.GetComponent<RectTransform>();
                        // RectTransform contentRect = lineTextContent.GetComponent<RectTransform>();
                        // newTextRect.localPosition = new Vector3(newTextRect.localPosition.x, newTextRect.localPosition.y, 0);
                        // newTextRect.localRotation = Quaternion.Euler(0f, 0f, 0f);
                        // newTextRect.localScale = new Vector3(1, 1, 1);
                    }
                    else
                    {
                ...
                if (useFadeEffect)
                {
                    ...
                    //Debug.Log("fading in");
                    yield return StartCoroutine(Effects.FadeAlpha(canvasGroup, 1, 1.0f, fadeInTime, currentStopToken));
        ...
        {
            GameObject newestLine = lineTextContent.transform.GetChild(lineTextContent.transform.childCount - 1).gameObject;
            TypewriterByCharacter typewriter = newestLine.GetComponent<TypewriterByCharacter>();
            if(typewriter != null)
            {
                ...
                else
                {
                    ...
                    DialogueManager.Instance.dialogueRunner.Dialogue.Continue();
        ...
        public override void DialogueComplete()
        {
            ...
            if (currentLine != null)
            {
                DOTween.To(() => canvasGroup.alpha, x => canvasGroup.alpha = x, 0f, fadeOutTime);
                currentLine = null;
                characterNameStored = null;
                StartCoroutine(DismissLineInternal(null));
            }
        }
        public void OnDialogueStart()
        {
            Debug.Log("line view fade in");
